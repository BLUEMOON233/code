...
class Student {
   public:
    Student(){};
    Student(std::string Tname, int Tid, std::string Tgender) : name(Tname), id(Tid), gender(Tgender) {}
    void setName(std::string tName) { this->name = tName; }
    void setID(int tID) { this->id = tID; }
    void setGender(std::string tGender) { this->gender = tGender; }
    ...
    bool operator==(Student& stu) {
        if (this->gender != stu.getGender() || this->name != stu.getName() || this->id != stu.getID()) return false;
        else return true;
    }
...
void LinkList<Element>::printList() {
    ...
    if (this->getLength() == 0) std::cout << "暂无学生数据！";
    else {
        // std::cout << "姓名" << ' ' << '\t' << "ID" << ' ' << '\t' << "性别" << '\n';
        while (now != nullptr) {
            std::cout << now->data << ' ';
