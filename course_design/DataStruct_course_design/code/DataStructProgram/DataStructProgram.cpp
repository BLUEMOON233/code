	...
	: QMainWindow(parent), ui(new Ui::DataStructProgramClass)
{
	...
	connect(ui->systemOpenFile, &QPushButton::clicked, [=]() mutable {
		QString filePath = QFileDialog::getOpenFileName(this, QString::fromLocal8Bit(std::string("选择文件").data()), QString::fromLocal8Bit(std::string("D:/DataStruct/data").data()));
		if (filePath.isEmpty()) {
			QMessageBox::information(NULL, QString::fromLocal8Bit(std::string("错误").data()), QString::fromLocal8Bit(std::string("路径为空").data()), QMessageBox::Ok | QMessageBox::Cancel, QMessageBox::Ok);
			return;
		}
	...
	connect(ui->systemSaveFile, &QPushButton::clicked, [=]()mutable {
		QString filePath = QFileDialog::getExistingDirectory(this, QString::fromLocal8Bit(std::string("选择文件夹").data()), "D:/DataStruct/data");
		if (filePath.isEmpty()) {
			QMessageBox::information(NULL, QString::fromLocal8Bit(std::string("错误").data()), QString::fromLocal8Bit(std::string("路径为空").data()), QMessageBox::Ok | QMessageBox::Cancel, QMessageBox::Ok);
			return;
		}
	...
	connect(ui->P2_PBopenFile, &QPushButton::clicked, [=]() mutable {
		...
		if (majorName == "") {
			QMessageBox::information(NULL, QString::fromLocal8Bit(std::string("错误").data()), QString::fromLocal8Bit(std::string("专业名为空").data()), QMessageBox::Ok | QMessageBox::Cancel, QMessageBox::Ok);
			return;
		}
		if (classNumData.isEmpty()) {
			QMessageBox::information(NULL, QString::fromLocal8Bit(std::string("错误").data()), QString::fromLocal8Bit(std::string("班级数量为空").data()), QMessageBox::Ok | QMessageBox::Cancel, QMessageBox::Ok);
			return;
		}
		if (stuNumData.isEmpty()) {
			QMessageBox::information(NULL, QString::fromLocal8Bit(std::string("错误").data()), QString::fromLocal8Bit(std::string("班级人数为空").data()), QMessageBox::Ok | QMessageBox::Cancel, QMessageBox::Ok);
			return;
		}
		...
		QString filePath = QFileDialog::getOpenFileName(this, QString::fromLocal8Bit(std::string("选择文件").data()), QString::fromLocal8Bit(std::string("D:/DataStruct/data/大类分流软件").data()));
		if (filePath.isEmpty()) {
			QMessageBox::information(NULL, QString::fromLocal8Bit(std::string("错误").data()), QString::fromLocal8Bit(std::string("路径为空").data()), QMessageBox::Ok | QMessageBox::Cancel, QMessageBox::Ok);
			return;
		}
			...
			ui->tableWidget->setHorizontalHeaderLabels(QStringList() << QString::fromLocal8Bit(std::string("编号").data()) << QString::fromLocal8Bit(std::string("志愿").data()) << QString("GPA") << QString::fromLocal8Bit(std::string("排名").data()));
	...
	connect(ui->P2_PBstartDiver, &QPushButton::clicked, [=]() mutable {
		if (DSTS.isInit) {
			QMessageBox::information(NULL, QString::fromLocal8Bit(std::string("错误").data()), QString::fromLocal8Bit(std::string("已有分流后数据").data()), QMessageBox::Ok | QMessageBox::Cancel, QMessageBox::Ok);
			return;
		}
		std::vector<double> grade = DSTS.initSystem(DSP);
		std::vector<int> classNum = DSTS.getClassNum();
		ui->tableWidget->setColumnCount(3);
		ui->tableWidget->setHorizontalHeaderLabels(QStringList() << QString::fromLocal8Bit(std::string("专业").data()) << QString::fromLocal8Bit(std::string("班级").data()) << QString::fromLocal8Bit(std::string("GPA均值").data()));
	...
	connect(ui->P2_PBQuery, &QPushButton::clicked, [=]() mutable {
		if (!DSTS.isInit) {
			QMessageBox::information(NULL, QString::fromLocal8Bit(std::string("错误").data()), QString::fromLocal8Bit(std::string("未检测到分流后数据").data()), QMessageBox::Ok | QMessageBox::Cancel, QMessageBox::Ok);
			return;
		}
		int nowID = ui->P2_LE_IDQuery->text().toInt();
		D_S_StudentPro now = DSTS.getArrayNodeByID(nowID);
		if (now.ID == -1) {
			QMessageBox::information(NULL, QString::fromLocal8Bit(std::string("错误").data()), QString::fromLocal8Bit(std::string("未找到该学号学生").data()), QMessageBox::Ok | QMessageBox::Cancel, QMessageBox::Ok);
			return;
		}
	...
	connect(ui->P2_PBModify, &QPushButton::clicked, [=]() mutable {
		...
		if (ui->P2_LEmajorModify->text().isEmpty()) {
			QMessageBox::information(NULL, QString::fromLocal8Bit(std::string("错误").data()), QString::fromLocal8Bit(std::string("专业名不可以为空").data()), QMessageBox::Ok | QMessageBox::Cancel, QMessageBox::Ok);
			return;
		}
		if (ui->P2_LEclassModify->text().isEmpty()) {
			QMessageBox::information(NULL, QString::fromLocal8Bit(std::string("错误").data()), QString::fromLocal8Bit(std::string("班级不可以为空").data()), QMessageBox::Ok | QMessageBox::Cancel, QMessageBox::Ok);
			return;
		}
		int majorPos = DSTS.getMajorPos(std::string(ui->P2_LEmajorModify->text().toLocal8Bit()));
		if (majorPos == 0) {
			QMessageBox::information(NULL, QString::fromLocal8Bit(std::string("错误").data()), QString::fromLocal8Bit(std::string("专业名有误").data()), QMessageBox::Ok | QMessageBox::Cancel, QMessageBox::Ok);
			return;
		}
		...
		if (ui->P2_LEclassModify->text().toInt() < classPos.first + 1 || ui->P2_LEclassModify->text().toInt() > classPos.first + classPos.second) {
			QMessageBox::information(NULL, QString::fromLocal8Bit(std::string("错误").data()), QString::fromLocal8Bit(std::string("班级号有误").data()), QMessageBox::Ok | QMessageBox::Cancel, QMessageBox::Ok);
			return;
		}
	...
	connect(ui->mainPB3, &QPushButton::clicked, [=]() mutable {
		...
		if (!DSTS.isInit) {
			...
			ui->P3_tableWidget->setHorizontalHeaderLabels(QStringList() << QString::fromLocal8Bit(std::string("未检测到分流后学生列表").data()));
			return;
		}
		std::vector<D_S_StudentPro> DSSarray = DSTS.getArray();
		ui->P3_tableWidget->setColumnCount(5);
		ui->P3_tableWidget->setHorizontalHeaderLabels(QStringList() << QString::fromLocal8Bit(std::string("专业").data()) << QString::fromLocal8Bit(std::string("班级").data()) << QString::fromLocal8Bit(std::string("排名").data()) << QString::fromLocal8Bit(std::string("学号").data()) << QString::fromLocal8Bit(std::string("绩点").data()));
	...
	connect(ui->P4_PBanalyse, &QPushButton::clicked, [=]()mutable {
		...
		if (!(qry_majorStr.empty() ^ qry_classStr.empty())) {
			QMessageBox::information(NULL, QString::fromLocal8Bit(std::string("错误").data()), QString::fromLocal8Bit(std::string("查询值异常").data()), QMessageBox::Ok | QMessageBox::Cancel, QMessageBox::Ok);
			return;
		}
		if (!qry_classStr.empty()) {
			...
			ui->P4_tableWidget->setHorizontalHeaderLabels(QStringList() << QString::fromLocal8Bit(std::string("专业").data()) << QString::fromLocal8Bit(std::string("班级").data()) << QString::fromLocal8Bit(std::string("GPA均值").data()) << QString::fromLocal8Bit(std::string("GPA最大值").data()) << QString::fromLocal8Bit(std::string("GPA最小值").data()) << QString::fromLocal8Bit(std::string("方差").data()));
			for (auto& x : intVec) {
		...
		if (!qry_majorStr.empty()) {
			...
			ui->P4_tableWidget->setHorizontalHeaderLabels(QStringList() << QString::fromLocal8Bit(std::string("专业").data()) << QString::fromLocal8Bit(std::string("GPA均值").data()) << QString::fromLocal8Bit(std::string("GPA最大值").data()) << QString::fromLocal8Bit(std::string("GPA最小值").data()) << QString::fromLocal8Bit(std::string("方差").data()));
			for (auto& x : strVec) {
